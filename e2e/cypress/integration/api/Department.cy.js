...
context('Department', () => {
  ...
  it('lists departments', () => {
      cy.request('/departments').then(response => {
        ...
        cy.request('/departments?exists[municipalities.boulderAreas.rocks.boulders]=true').then(response => {
  ...
  it('cannot delete a department', () => {
      cy.request({url :'/departments/1', method: 'DELETE', failOnStatusCode: false}).then(response => {
          expect(response.status).to.eq(405)
      })
  })
 
  it('cannot create a department', () => {
      cy.request({url :'/departments', method: 'POST', body: {}, failOnStatusCode: false}).then(response => {
          expect(response.status).to.eq(405)
      })
  })

  it('cannot edit a department', () => {
      cy.request({url :'/departments/1', method: 'PUT', body: {}, failOnStatusCode: false}).then(response => {
          expect(response.status).to.eq(405)
      })

      cy.request({url :'/departments/1', method: 'PATCH', body: {}, failOnStatusCode: false}).then(response => {
          expect(response.status).to.eq(405)
      })
  })
