...
context('LineBoulder', () => {
 ...
 it('deletes a line boulder', () => {
      ...
      cy.request({ method: 'DELETE', url: '/admin/line_boulders/1'}).then(response => {
  ...
  it('creates a line boulder', () => {
      ...
      cy.request({
          ...
          expect(response.body['hydra:description']).to.contain('boulder: Cette valeur ne doit pas être vide.')
          expect(response.body['hydra:description']).to.contain('smoothLine: Cette valeur ne doit pas être vide.')
          expect(response.body['hydra:description']).to.contain('arrArrPoints: Cette valeur ne doit pas être vide.')
      })
      ...
      cy.request({
          ...
          expect(response.status).to.eq(422)
          expect(response.body['hydra:description']).to.contain('boulder: This boulder does not match with its rock associated')
      })
