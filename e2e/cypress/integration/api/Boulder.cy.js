...
context("Boulder", () => {
  ...
  it("lists boulders", () => {
    cy.request("/boulders").then((response) => {
      ...
      expect(response.body["hydra:member"][0].rock.location.latitude).not.to.be
        .empty;
      expect(response.body["hydra:member"][0].rock.location.longitude).not.to.be
        .empty;
      expect(response.body["hydra:member"][0].rock.location.longitude).not.to.be
        .empty;
      expect(response.body["hydra:member"][0].rock.boulderArea.name).not.to.be
        .empty;
      expect(
        response.body["hydra:member"][0].rock.boulderArea.municipality.name
      ).not.to.be.empty;
      expect(response.body["hydra:member"][0].description).to.be.undefined;
      expect(
        response.body["hydra:member"][0].lineBoulders[0].rockImage.filterUrl
      ).to.contain("%filter%");
      expect(
    ...
    cy.request({
      url: "/boulders?pagination=false&groups[]=Boulder:read&groups[]=read&groups[]=Boulder:item-get",
    }).then((response) => {
      expect(response.body["hydra:member"][0]["name"]).to.eq("Stone");
      expect(response.body["hydra:member"][0]["description"]).to.eq(
        "Un rétablissement sur 2 bonnes réglettes"
      );
      expect(response.body["hydra:member"][0]["rock"]["location"]["latitude"])
        .not.to.be.null;
    });
  });
  ...
  it("gets a boulder", () => {
    cy.request("/boulders/1").then((response) => {
      ...
      expect(response.body.lineBoulders[0].rockImage.filterUrl).to.contain(
        "%filter%"
      );
  ...
  it("can search boulders by entering their name", () => {
    cy.request({ url: "/boulders?term=Onk", method: "GET" }).then(
      (response) => {
  ...
  it("can search boulders by entering a boulder area", () => {
    cy.request({ url: "/boulders?term=cre", method: "GET" }).then(
      (response) => {
  ...
  it("can search boulders by entering a municipality", () => {
    cy.request({ url: "/boulders?term=ker", method: "GET" }).then(
      (response) => {
        expect(response.body["hydra:totalItems"]).to.eq(4);
        cy.wrap(response.body["hydra:member"]).each((item) => {
          cy.wrap(item.rock.boulderArea.municipality.name).should(
            "eq",
            "Kerlouan"
          );
        });
    ...
    cy.request({ url: "/boulders?term=plaintel", method: "GET" }).then(
      (response) => {
