...
context('Rocks', () => {
  ...
  it('show details', () => {
    ...
    cy.get('dt').contains('Blocs').next().contains('Les cornes du diable')
  })
  ...
  it('create a rock', () => {
    ...
    cy.get('.cy-pictures input[type="file"]').selectFile({contents: '@boulder', fileName: 'boulder.jpg'});

    cy.get('button.action-save').contains('Créer').click()
    cy.get('table tbody tr').should('have.length', 4)
    cy.get('table tbody tr').first().find('.cy-pictures img').should('be.visible')
    cy.get('table tbody tr').first().find('.cy-pictures').should('contain.text', '1')
