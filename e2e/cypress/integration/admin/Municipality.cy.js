...
context('Municipality as admin', () => {
  ...
  it('show details', () => {
    cy.get('table tbody tr').contains('Kerlouan').closest('tr').find('a.dropdown-toggle').first().takeAction('Consulter')
    cy.get('h1').should('contain.text', 'Kerlouan')
    cy.get('dt').contains('Secteurs').next().find('tbody tr').should('have.length', 5).should('contain.text', 'Cremiou')
...
context('Municipality as super admin', () => {
  ...
  it('create a municipality', () => {
    ...
    cy.get('input[name=Municipality\\[name\\]]').should('have.class', 'is-invalid')
    cy.get('input[name=Municipality\\[code\\]]').should('have.class', 'is-invalid')

    cy.get('input[name=Municipality\\[name\\]]').type('Locmaria-Plouzané')
    cy.get('input[name=Municipality\\[code\\]]').type('29098')
    cy.get('button.action-save').contains('Créer').click()
    cy.get('input[name=Municipality\\[code\\]]').next().get('.invalid-feedback').should('contain.text', 'Cette valeur est déjà utilisée')
